...
public class MainMenu {
	...
	public static Employee employ(Scanner inter, Scanner stringer, Scanner doubler, int hashSize){
		...
		while(auxChoose >= 3 || auxChoose == 0){
			System.out.println("Is he/her with the sindicate?\n1)Yes 2)No");
			auxChoose = inter.nextInt();
		}
		boolean auxSindicate;
		if(auxChoose == 1) auxSindicate = true; 
		else auxSindicate = false;
		auxChoose = 4;
		...
		if(auxChoose == 1){
			...
			return new Hourly(auxName, auxAddress, auxPayment, hashSize, auxSindicate);
		}
		else if(auxChoose == 2){
			System.out.println(auxName + " got employed! Huzzah!");
			return new Salaried(auxName, auxAddress, auxPayment, hashSize, auxSindicate);
		}
		else{
			...
			System.out.println(auxName + " got employed! Huzzah!");
			return new Comissioned(auxName, auxAddress, auxPayment, auxPercentage, hashSize, auxSindicate);
		}
	}	
		...
		if(hash.containsKey(aux) == true){
			if(hash.get(aux) instanceof Hourly){
				...
				if(hours <=8){
					...
					hash.get(aux).calculate((hash.get(aux).getPayment()) * 8);					
					hash.get(aux).calculate((hash.get(aux).getPayment()*1.5)*(hours-8));
				}
	...
	public static void applyTax(Scanner stringer, Scanner doubler, HashMap<String, Employee> hash){
		System.out.println("Please write the employee's name below:");
		s
	}
